...
#should have gotten the above package a long time ago, but since I didn't, had to get it to do rtracklayer
```
...
#so we see here that 14 peaks overlap once which is a lot 
table(table(table(ovf@from)))
#frequency of number of overlaps happening from the last one (there were 2 instances of 8 overlaps)
table(table(table(table(ovf@from))))
#frequency of number of overlaps from last one
#get rid of unique values from double table 
...
# same for @to
table(ovf@to)
#looking at file 2 into file 1
table(table(ovf@to))
#almost all of them are single overlaps
